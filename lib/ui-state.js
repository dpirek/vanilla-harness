...
import { createSkillFileStore, skillsRootForDatabase } from "./skill-files.js";
import { createToolFileStore, toolsRootForDatabase } from "./tool-files.js";
...
function normalizeStoredToolPermissions(value = {}) {
  ...
  for (const [name, enabled] of Object.entries(value || {})) {
    if (/^[a-z][a-z0-9_]{0,63}$/.test(name) && typeof enabled === "boolean") defaults[name] = enabled;
  }
  return defaults;
}

function normalizeStoredProviderModels(value) {
  ...
  for (const entry of Array.isArray(value) ? value : []) {
    const id = typeof entry === "string"
      ...
      : entry && typeof entry === "object"
        ...
        : "";
    if (!id) continue;
    if (!models.has(id) || typeof entry === "object") {
      models.set(id, typeof entry === "string" ? id : { ...entry, id });
    }
  }
...
function normalizeRigConfiguration(config = {}, index = 0) {
  return {
    ...
    skillAutoDiscovery: config.skillAutoDiscovery !== false,
...
export function createUiStateStore(databasePath) {
  ...
  const skillFiles = createSkillFileStore(skillsRootForDatabase(databasePath));
  const toolFiles = createToolFileStore(toolsRootForDatabase(databasePath));
  database.exec("CREATE TABLE IF NOT EXISTS runtime_values (key TEXT PRIMARY KEY, value TEXT NOT NULL) STRICT");
  database.exec(`
  ...
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS presets (
      ...
      skill_auto_discovery INTEGER NOT NULL DEFAULT 1 CHECK (skill_auto_discovery IN (0, 1)),
    ...
    CREATE INDEX IF NOT EXISTS task_ratings_provider_model
      ON task_ratings(provider_id, model);
  ...
  const providerSettingsColumns = database.prepare(
    "SELECT name FROM pragma_table_info('provider_settings')",
  ).all().map(({ name }) => name);
  if (!providerSettingsColumns.includes("models")) {
    database.exec("ALTER TABLE provider_settings ADD COLUMN models TEXT NOT NULL DEFAULT '[]'");
  }
  if (!providerSettingsColumns.includes("models_loaded_at")) {
    database.exec("ALTER TABLE provider_settings ADD COLUMN models_loaded_at INTEGER");
  }
  ...
  if (!presetColumns.includes("skill_auto_discovery")) {
    database.exec("ALTER TABLE presets ADD COLUMN skill_auto_discovery INTEGER NOT NULL DEFAULT 1 CHECK (skill_auto_discovery IN (0, 1))");
  }
  ...
  const legacySourcePaths = skillColumns.includes("source_path")
    ? new Map(database.prepare("SELECT id, source_path FROM skills").all().map((row) => [String(row.id), row.source_path]))
    : new Map();
  ...
  if (addedPresetSkillIds) {
    const selectedSkillIds = skillColumns.length
      ? database.prepare("SELECT id FROM skills WHERE selected = 1 ORDER BY id").all().map(({ id }) => id)
      : [];
    database.prepare("UPDATE presets SET skill_ids = ?").run(JSON.stringify(selectedSkillIds));
  }
  if (skillColumns.length) {
    const legacy = database.prepare("SELECT id, name, content, selected FROM skills ORDER BY id").all();
    const idMap = new Map();
    const selectedIds = [];
    const existingSkills = new Map(skillFiles.list().map((entry) => [entry.id, entry.content]));
    const nameCounts = new Map();
    for (const row of legacy) {
      const base = normalizeSkillName(row.name) || `skill-${String(row.id).slice(0, 8)}`;
      nameCounts.set(base, (nameCounts.get(base) || 0) + 1);
    }
    for (const row of legacy) {
      const base = normalizeSkillName(row.name) || `skill-${String(row.id).slice(0, 8)}`;
      const suffix = nameCounts.get(base) > 1 ? `-${crypto.createHash("sha256").update(String(row.id)).digest("hex").slice(0, 8)}` : "";
      let name = `${base.slice(0, 63 - suffix.length).replace(/-+$/, "")}${suffix}`;
      let index = 2;
      const legacyContent = String(row.content || "");
      const prepared = (skillName) => {
        try { return validateSkillContent(syncSkillContentName(legacyContent, skillName)); }
        catch { return skillDraft(skillName).replace("Describe the exact workflow the agent should follow.\n", `${legacyContent}\n`); }
      };
      while (existingSkills.has(name) && existingSkills.get(name) !== prepared(name)) {
        const suffix = `-${index++}`;
        name = `${base.slice(0, 63 - suffix.length).replace(/-+$/, "")}${suffix}`;
      }
      const content = prepared(name);
      if (!existingSkills.has(name)) skillFiles.write(name, content, { create: true });
      existingSkills.set(name, content);
      const sourcePath = legacySourcePaths.get(String(row.id)) || (fs.existsSync(String(row.id)) ? String(row.id) : "");
      if (sourcePath && fs.existsSync(sourcePath) && !fs.lstatSync(sourcePath).isSymbolicLink()) {
        ...
        const copyResources = (directory, prefix) => {
          if (!fs.existsSync(directory)) return;
          for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
            if (entry.isSymbolicLink()) continue;
            const relative = `${prefix}/${entry.name}`;
            const target = path.join(directory, entry.name);
            if (entry.isDirectory()) copyResources(target, relative);
            else if (entry.isFile() && !fs.existsSync(path.join(skillFiles.root, name, relative))) {
              skillFiles.writeResource(name, relative, fs.readFileSync(target));
            }
          }
        };
        for (const folder of ["scripts", "references", "templates", "examples"]) {
          copyResources(path.join(sourceDir, folder), folder);
        }
      }
      idMap.set(String(row.id), name);
      if (row.selected) selectedIds.push(name);
    }
    database.exec("BEGIN IMMEDIATE");
    try {
      const presets = database.prepare("SELECT id, skill_ids FROM presets").all();
      const update = database.prepare("UPDATE presets SET skill_ids = ? WHERE id = ?");
      for (const preset of presets) {
        const ids = JSON.parse(preset.skill_ids || "[]").map((id) => idMap.get(String(id)) || String(id));
        update.run(JSON.stringify([...new Set(ids.length ? ids : selectedIds)]), preset.id);
      }
      database.exec("ALTER TABLE skills RENAME TO skills_legacy_archive");
  ...
  const updateProviderSettings = database.prepare(`
    INSERT INTO provider_settings (provider_id, model, models, models_loaded_at, selected, updated_at) VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT(provider_id) DO UPDATE SET model = excluded.model,
      models = excluded.models, models_loaded_at = excluded.models_loaded_at,
      selected = excluded.selected, updated_at = excluded.updated_at
  `);
  const insertPreset = database.prepare(`
    INSERT INTO presets
      (id, name, component_state, system_prompts, provider_settings, tool_permissions, skill_ids, skill_auto_discovery, mcp_config, sub_agents, updated_at, sort_order, selected)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const readPresetRows = database.prepare(`
    SELECT id, name, component_state, system_prompts, provider_settings, tool_permissions, skill_ids, skill_auto_discovery, mcp_config, sub_agents, updated_at, selected
  ...
  `);
  const upsertTaskRating = database.prepare(`
    INSERT INTO task_ratings
      (run_id, session_id, provider_id, provider_name, model, input_prompt, preset_settings,
       tools, system_prompts, cost, rating, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(run_id) DO UPDATE SET
      ...
      preset_settings = excluded.preset_settings,
  ...
  const readTaskRatings = (full = false) => database.prepare(`
    SELECT run_id, session_id, provider_id, provider_name, model, cost, rating,
      created_at, updated_at${full ? ", input_prompt, preset_settings, tools, system_prompts" : ""}
    FROM task_ratings ORDER BY updated_at DESC
  `).all().map((row) => ({
    runId: row.run_id,
    ...
    ...(full ? {
      ...
      presetSettings: JSON.parse(row.preset_settings),
  ...
  const readSkills = () => {
    const active = database.prepare("SELECT skill_ids FROM presets WHERE selected = 1").get();
    const selected = new Set(JSON.parse(active?.skill_ids || "[]"));
    return skillFiles.list().map((skill) => ({ ...skill, selected: selected.has(skill.id) }));
  };
  ...
  const readProviders = () => database.prepare(`
    SELECT p.id, p.name, p.type, p.base_url, p.api_key, ps.model, ps.models, ps.models_loaded_at, ps.selected
    FROM providers p JOIN provider_settings ps ON ps.provider_id = p.id
  ...
  `).all().map((row) => ({
    ...
    modelsLoadedAt: row.models_loaded_at,
  ...
  const setCurrentProviderSettings = (settings, now = Date.now()) => {
    ...
    updateProviderSettings.run(
      providerId,
      normalized.model,
      JSON.stringify(existing?.models || []),
      existing?.modelsLoadedAt || null,
      1,
      now,
    );
    return normalized;
  ...
  const readPresets = () => readPresetRows.all().map((row) => ({
    ...
    skillAutoDiscovery: row.skill_auto_discovery === 1,
  ...
  const applyRigConfigurationSnapshot = (configuration, { syncProviderSettings = true } = {}) => {
    ...
    // The active preset's skill IDs select folders in /skills.
  ...
  const replaceRigConfigurations = (configurations = [], activeConfigurationId = null, options = {}) => {
    ...
    normalized.forEach((configuration, index) => {
      insertPreset.run(
        ...
        configuration.skillAutoDiscovery ? 1 : 0,
  ...
  function writeValues(values) {
    ...
    if (Object.hasOwn(values, "sessions")) {
      const nextIds = new Set((values.sessions || []).map((session) => String(session.id)));
      for (const session of database.prepare('SELECT id, workspace FROM sessions').all()) {
        if (nextIds.has(session.id)) continue;
        const key = `context:${crypto.createHash('sha256').update(`${path.resolve(session.workspace)}\0${session.id}`).digest('hex')}`;
        database.prepare('DELETE FROM runtime_values WHERE key = ? OR substr(key, 1, ?) = ?').run(key, key.length + 1, `${key}:`);
      }
    ...
    if (Array.isArray(values.providers)) {
      ...
      for (const item of values.providers) {
        ...
        const models = normalizeStoredProviderModels(item.models);
        updateProviderSettings.run(
          id,
          String(item.model || ""),
          JSON.stringify(models),
          Number(item.modelsLoadedAt) || null,
  ...
  return {
    deleteRuntimePrefix(prefix) {
      database.prepare("DELETE FROM runtime_values WHERE substr(key, 1, ?) = ?").run(prefix.length, prefix);
    },
    getRuntimeValue(key) {
      const row = database.prepare("SELECT value FROM runtime_values WHERE key = ?").get(key);
      return row ? JSON.parse(row.value) : undefined;
    },
    setRuntimeValue(key, value) {
      database.prepare("INSERT INTO runtime_values VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(key, JSON.stringify(value));
    },
    getAll() {
    ...
    getTools() {
      return toolFiles.list();
    },
    getToolRoot() {
      return toolFiles.root;
    },
    getTool(name) {
      return toolFiles.read(name);
    },
    createTool(content) {
      return toolFiles.write(content, { create: true });
    },
    updateTool(content) {
      return toolFiles.write(content);
    },
    testTool(content, source) {
      return toolFiles.test(content, source);
    },
    importToolFiles(files) {
      return toolFiles.importFiles(files);
    },
    getToolResource(name, resource) {
      return toolFiles.readResource(name, resource);
    },
    setToolResource(name, resource, content) {
      toolFiles.writeResource(name, resource, content);
    },
    getSkills() {
      return readSkills();
    },
    getSkillCatalog() {
      const selected = new Set(readPresets().find((item) => item.selected)?.skillIds || []);
      return skillFiles.catalog().map((skill) => ({ ...skill, selected: selected.has(skill.id) }));
    },
    getSkillAutoDiscovery() {
      return readPresets().find((item) => item.selected)?.skillAutoDiscovery !== false;
    },
    getSkill(skillId) {
      return skillFiles.read(skillId);
    },
    getSelectedSkills() {
    ...
    createSkill({ name, content }) {
      ...
      skillFiles.write(normalizedName, content, { create: true });
      const skills = readSkills();
      return { skill: skills.find((skill) => skill.id === normalizedName), skills };
    },
    updateSkill(skillId, { name, content }) {
      ...
      skillFiles.rename(id, normalizedName, content);
      if (id !== normalizedName) {
        const presets = database.prepare("SELECT id, skill_ids FROM presets").all();
        const update = database.prepare("UPDATE presets SET skill_ids = ? WHERE id = ?");
        for (const preset of presets) {
          const ids = JSON.parse(preset.skill_ids).map((value) => value === id ? normalizedName : value);
          update.run(JSON.stringify(ids), preset.id);
        }
      }
      const updatedSkills = readSkills();
      return { skill: updatedSkills.find((skill) => skill.id === normalizedName), skills: updatedSkills };
    },
    setSelectedSkills(skillIds = [], skillAutoDiscovery) {
      ...
      if (active) {
        this.setRigConfigurations(
          readPresets().map((configuration) => (
            configuration.id === active.id
              ? { ...configuration, skillIds: ids, skillAutoDiscovery: typeof skillAutoDiscovery === "boolean" ? skillAutoDiscovery : configuration.skillAutoDiscovery, updatedAt: Date.now() }
              : configuration
          )),
          active.id,
        );
    ...
    importSkill(files) {
      const skill = skillFiles.importFiles(files);
      return { skill, skills: readSkills() };
    },
    getSkillResource(skillId, resourcePath) {
      return skillFiles.readResource(skillId, resourcePath);
    },
    setSkillResource(skillId, resourcePath, content) {
      skillFiles.writeResource(skillId, resourcePath, content);
      return { skills: readSkills() };
    },
    testSkill(skillId) {
      return skillFiles.test(skillId);
    },
    getTaskRatings(options = {}) {
      return readTaskRatings(options.full === true);
    },
    setTaskRating(record = {}) {
      const runId = String(record.runId || "").trim();
      const sessionId = String(record.sessionId || "").trim();
      const providerName = String(record.providerName || record.provider || "").trim();
      const model = String(record.model || "").trim();
      const rating = Number(record.rating);
      if (!runId || !sessionId || !providerName || !model) {
        throw new Error("Run, session, provider, and model are required for a task rating.");
      }
      if (!Number.isInteger(rating) || rating < 1 || rating > 5) {
        throw new Error("Task rating must be an integer from 1 to 5.");
      }
      const now = Date.now();
      const existing = database.prepare("SELECT created_at FROM task_ratings WHERE run_id = ?").get(runId);
      const cost = record.cost === null || record.cost === undefined ? null : Number(record.cost);
      upsertTaskRating.run(
        runId,
        sessionId,
        String(record.providerId || ""),
        providerName,
        model,
        String(record.inputPrompt || ""),
        JSON.stringify(record.presetSettings && typeof record.presetSettings === "object" ? record.presetSettings : {}),
        JSON.stringify(record.tools && typeof record.tools === "object" ? record.tools : {}),
        JSON.stringify(record.systemPrompts && typeof record.systemPrompts === "object" ? record.systemPrompts : {}),
        Number.isFinite(cost) && cost >= 0 ? cost : null,
        rating,
        existing?.created_at || now,
        now,
      );
      return readTaskRatings(false).find((entry) => entry.runId === runId);
    },
  };
}
