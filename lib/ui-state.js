...
export function createUiStateStore(databasePath) {
  ...
  database.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS one_selected_rig_configuration
      ON rig_configurations(selected) WHERE selected = 1;

    INSERT OR IGNORE INTO layout (id, sidebar_collapsed) VALUES (1, 0);
  `);
  ...
  const rigConfigurationColumns = database.prepare(
    "SELECT name FROM pragma_table_info('rig_configurations')",
  ).all().map(({ name }) => name);
  if (!rigConfigurationColumns.includes("provider_settings")) {
    database.exec("ALTER TABLE rig_configurations ADD COLUMN provider_settings TEXT NOT NULL DEFAULT '{}'");
  }
  ...
  const insertTokenUsage = database.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const updateActiveSession = database.prepare(
    "UPDATE layout SET active_session_id = ? WHERE id = 1",
  );
  const updateSidebar = database.prepare(
    "UPDATE layout SET sidebar_collapsed = ? WHERE id = 1",
  );
  ...
  const upsertPermission = database.prepare(`
    INSERT INTO tool_permissions (name, enabled) VALUES (?, ?)
    ON CONFLICT(name) DO UPDATE SET enabled = excluded.enabled
  `);
  const insertRigConfiguration = database.prepare(`
    INSERT INTO rig_configurations
      (id, name, component_state, system_prompts, provider_settings, tool_permissions, mcp_config, updated_at, sort_order, selected)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const readRigConfigurationRows = database.prepare(`
    SELECT id, name, component_state, system_prompts, provider_settings, tool_permissions, mcp_config, updated_at, selected
    FROM rig_configurations ORDER BY sort_order
  `);
  ...
  const readCurrentToolPermissions = () => {
    const rows = database.prepare("SELECT name, enabled FROM tool_permissions").all();
    return normalizeStoredToolPermissions(Object.fromEntries(rows.map(({ name, enabled }) => [name, enabled === 1])));
  };

  const readSkills = () => database.prepare(`
    SELECT id, name, source_path, content, selected, updated_at
    FROM skills
    ORDER BY selected DESC, lower(name), source_path
  `).all().map((row) => ({
    id: row.id,
    name: row.name,
  ...
  const readRigConfigurations = () => readRigConfigurationRows.all().map((row) => ({
    id: row.id,
    name: row.name,
  ...
  const applyRigConfigurationSnapshot = (configuration, { syncProviderSettings = true } = {}) => {
    ...
    database.exec("DELETE FROM tool_permissions");
    for (const [name, enabled] of Object.entries(normalizeStoredToolPermissions(configuration.toolPermissions))) {
      upsertPermission.run(name, enabled === true ? 1 : 0);
    }
  ...
  const buildDefaultRigConfiguration = (name = "Default") => ({
    ...
    toolPermissions: readCurrentToolPermissions(),
  ...
  const replaceRigConfigurations = (configurations = [], activeConfigurationId = null, options = {}) => {
    ...
    database.exec("DELETE FROM rig_configurations");
    normalized.forEach((configuration, index) => {
      insertRigConfiguration.run(
        configuration.id,
        configuration.name,
  ...
  function writeValues(values) {
    ...
    if (Object.hasOwn(values, "activeSessionId")) {
      updateActiveSession.run(values.activeSessionId == null ? null : String(values.activeSessionId));
    }
    if (Object.hasOwn(values, "sidebarCollapsed")) {
      updateSidebar.run(values.sidebarCollapsed === true ? 1 : 0);
    }
    if (Array.isArray(values.providers)) {
      ...
      const active = readRigConfigurations().find((configuration) => configuration.selected);
      if (active) {
        replaceRigConfigurations(
          readRigConfigurations().map((configuration) => (
    ...
    if (values.toolPermissions && typeof values.toolPermissions === "object") {
      for (const [name, enabled] of Object.entries(values.toolPermissions)) {
        upsertPermission.run(name, enabled === true ? 1 : 0);
      }
      const active = readRigConfigurations().find((configuration) => configuration.selected);
      if (active) {
  ...
  if (readRigConfigurations().length === 0) {
    database.exec("BEGIN IMMEDIATE");
    try {
  ...
  return {
    getAll() {
      ...
      const layout = database.prepare(
        "SELECT active_session_id, sidebar_collapsed FROM layout WHERE id = 1",
      ).get();
      const providers = readProviders();
      const provider = providers.find((item) => item.selected);
      const permissions = database.prepare("SELECT name, enabled FROM tool_permissions").all();
      return {
        ...(sessions.length > 0 ? { sessions } : {}),
        ...(layout?.active_session_id ? { activeSessionId: layout.active_session_id } : {}),
        sidebarCollapsed: layout?.sidebar_collapsed === 1,
        ...
        ...(permissions.length > 0 ? { toolPermissions: Object.fromEntries(
          permissions.map(({ name, enabled }) => [name, enabled === 1]),
        ) } : {}),
      };
    },
    set(values) {
    ...
    getRigConfigurations() {
      const configurations = readRigConfigurations();
      return {
        configurations,
    ...
    syncSkills(skills = []) {
      ...
      const normalized = entries.map((skill) => ({
        id: String(skill.id),
        name: String(skill.name || skill.id),
        sourcePath: String(skill.sourcePath || skill.path || ""),
      ...
      database.exec("BEGIN IMMEDIATE");
      try {
        const existing = new Set(readSkills().map((skill) => skill.id));
        const next = new Set(normalized.map((skill) => skill.id));
        ...
        for (const id of existing) {
          if (!next.has(id)) deleteSkill.run(id);
        }
        database.exec("COMMIT");
      } catch (error) {
        database.exec("ROLLBACK");
