...
export async function serveStatic(req, res, publicDir) {
  ...
  const appRoute = /^\/models(?:\/[^/]+)?\/?$/.test(requestUrl.pathname) || /^\/tools(?:\/(?:permissions|files|runtime|test))?\/?$/.test(requestUrl.pathname) || /^\/(?:providers|skills|presets|system-prompts|mcp|workflow)\/?$/.test(requestUrl.pathname);
  const filePath = resolvePublicPath(publicDir, appRoute ? "/" : requestUrl.pathname);
  if (!filePath) {
