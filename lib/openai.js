...
class OpenAIClient {
  ...
  async createResponse(body, { onTextDelta, signal } = {}) {
    if (onTextDelta) return this.createStreamingResponse(body, { onTextDelta, signal });
  ...
  async createStreamingResponse(body, { onTextDelta, signal }) {
    const headers = { "content-type": "application/json" };
    if (this.apiKey) headers.authorization = `Bearer ${this.apiKey}`;
    const response = await this.fetch(`${this.baseUrl}/responses`, {
      signal,
...
function chatInputMessages(input = []) {
  return input.flatMap((item) => {
    ...
    if (message.role === 'assistant' && previous?.role === 'assistant' && message.tool_calls) {
...
class OllamaClient {
  constructor({ apiKey = "", baseUrl = "http://localhost:11434", fetchImpl = fetch }) {
    this.apiKey = apiKey;
  ...
  async createResponse(body, { onTextDelta, signal } = {}) {
    if (!body.previous_response_id) this.messages = [{ role: "system", content: body.instructions || "" }];
    this.messages.push(...chatInputMessages(body.input).map((message) => {
      if (message.tool_calls) return { ...message, tool_calls: message.tool_calls.map((call) => ({ ...call, function: { ...call.function, arguments: JSON.parse(call.function.arguments || "{}") } })) };
      if (!Array.isArray(message.content)) return message;
      return { ...message, content: message.content.filter((part) => part.type === 'text').map((part) => part.text).join('\n'), images: message.content.filter((part) => part.type === 'image_url').map((part) => base64Image(part.image_url.url)) };
    }));
    ...
    const requestBody = {
      ...
      ...(body.max_output_tokens ? { options: { num_predict: body.max_output_tokens } } : {}),
    };
...
function createModelClient({
  ...
  if (normalizedProvider === "ollama") {
    return new OllamaClient({
      apiKey: resolveProviderApiKey("ollama", apiKey),
