...
function mcpSpawnEnv(env = process.env) {
  const pathKey = Object.keys(env).find((key) => key.toLowerCase() === "path") || "PATH";
  const entries = String(env[pathKey] || "").split(path.delimiter).filter(Boolean);
  const fallbackEntries = process.platform === "win32"
    ? [path.dirname(process.execPath)]
    : [path.dirname(process.execPath), "/usr/local/bin", "/usr/bin", "/bin", "/snap/bin"];
  const executablePath = [...new Set([...entries, ...fallbackEntries])].join(path.delimiter);
  return { ...env, [pathKey]: executablePath };
}
...
function parseTomlConfig(raw, source) {
  ...
  raw.split(/\r?\n/).forEach((rawLine, index) => {
    ...
    const key = keyValue[1] === undefined
      ? keyValue[2]
      : keyValue[1].replace(/\\"/g, '"').replace(/\\\\/g, "\\");
    current[key] = parseTomlValue(keyValue[3], source);
  });
...
function normalizeRemoteServer(server, index, source, env, approvalDefault = "always") {
  ...
  if (normalized.headers !== undefined) {
    assertPlainObject(normalized.headers, `${source} MCP server "${label}" headers`);
    for (const [name, value] of Object.entries(normalized.headers)) {
      if (!/^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(name) || typeof value !== "string") {
        throw new Error(`MCP server "${label}" headers must map valid HTTP header names to strings.`);
      }
    }
  }
...
function normalizeInputSchema(schema) {
  ...
  if (schema.type === "object") {
    ...
    // Function tool schemas must be objects and cannot use these constraints at
    // the root, even though they are otherwise valid JSON Schema. Nested uses
...
class McpStdioClient {
  ...
  connect() {
    ...
    const childEnv = mcpSpawnEnv({ ...process.env, ...this.env, ...(this.server.env || {}) });
    ...
    this.child.on("error", (cause) => {
      ...
      const detail = missing
        ? `command "${this.server.command}" was not found. Install it or configure its absolute path` +
          (this.server.command === "npx" ? "; on Ubuntu, npx is provided by npm" : "") + "."
        : cause.message;
      rejectPending(new Error(
        `Could not start MCP server "${this.server.server_label}": ${detail}`,
        { cause },
      ));
    });
...
async function loadMcpTools({
  ...
  for (const { source, config } of configs) {
    ...
    for (const [index, server] of remoteMcpServers(config, source).entries()) {
      ...
      if (tool.server_url && (forceLocal || isLoopbackUrl(tool.server_url))) {
...
export { loadMcpTools, mcpSpawnEnv, mcpStartupTimeoutMs };
