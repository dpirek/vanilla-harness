...
import { createChangeHistory, workspacePath } from "../change-history.js";
...
function createToolRegistry(initialFactories = []) {
  ...
  function createTools({ root, approve = async () => false, subAgentManager, authorize = async () => true, store, settings } = {}) {
    ...
    const context = { ...createWorkspaceContext({ root, approve }), subAgentManager, changes, settings, authorize };
    return [...factories].map(([name, factory]) => {
      ...
      return { ...tool, async execute(args, options = {}) {
        ...
        } else await authorize(name, String(args?.command ?? args?.url ?? args?.agent ?? args?.action ?? '*'), { ...options, details: args });
