...
const RESOURCE_DIRS = new Set(["scripts", "references", "templates", "examples", "assets", "agents"]);
...
function resourcePath(value) {
  ...
  if (relative === "SKILL.md" || relative === "LICENSE.txt" || relative === "license.txt") return relative;
  if (parts.length < 2 || !RESOURCE_DIRS.has(parts[0]) || parts.some((part) => !part || part === "." || part === ".." || /[\x00-\x1f:]/.test(part))) {
    throw new Error("Resource paths must be inside scripts/, references/, templates/, examples/, assets/, or agents/.");
  }
  return relative;
}
...
function fileFor(root, name, relative) {
  ...
  if (fs.existsSync(parent) && fs.lstatSync(parent).isSymbolicLink()) throw new Error("Skill folders cannot follow symlinks.");
...
export function createSkillFileStore(root) {
  ...
  const catalog = () => fs.readdirSync(root, { withFileTypes: true })
    ...
    .map((entry) => {
      ...
      // Discovery reads only the frontmatter. Full instructions stay on disk until selected.
      const descriptor = fs.openSync(file, "r");
      const buffer = Buffer.alloc(Math.min(stat.size, 64_000));
      try { fs.readSync(descriptor, buffer, 0, buffer.length, 0); }
      finally { fs.closeSync(descriptor); }
      const excerpt = buffer.toString("utf8");
      let description = "";
      let valid = true;
      try { description = metadata(excerpt).description; }
      catch { valid = false; }
      return { id: entry.name, name: entry.name, description, path: file, valid, updatedAt: stat.mtimeMs };
    }).filter(Boolean).sort((a, b) => a.name.localeCompare(b.name));
  const read = (name) => {
    const id = skillName(name);
    const file = fileFor(root, id, "SKILL.md");
    if (!fs.existsSync(file)) throw new Error(`Unknown skill: ${id}`);
    const stat = fs.statSync(file);
    if (!stat.isFile() || stat.size > MAX_FILE_BYTES) throw new Error(`Skill guide is unavailable or too large: ${id}`);
    const content = fs.readFileSync(file, "utf8");
    const info = metadata(content);
    if (info.name !== id) throw new Error(`Skill frontmatter name does not match folder: ${id}`);
    return { id, name: id, description: info.description, path: file, valid: true, updatedAt: stat.mtimeMs, content, resources: listFiles(path.join(root, id)) };
  };
  ...
  function test(name) {
    ...
    for (const match of skill.content.matchAll(/\]\(([^)#]+)(?:#[^)]*)?\)/g)) {
      ...
      if (!/^(scripts|references|templates|examples|assets|agents)\//.test(reference)) continue;
  ...
  return { root, catalog, read, list, write, rename, readResource, writeResource, importFiles, test };
}
