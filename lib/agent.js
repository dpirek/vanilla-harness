...
import { relevantSkills, skillCatalogInstructions } from "./skill-discovery.js";
...
class CodingAgent {
  constructor({
    ...
    this.skillAutoDiscovery = skillAutoDiscovery !== false;
  ...
  async runInternal(prompt, { disabledSteps = [...this.disabledSteps], executionControl } = {}) {
    ...
    const matches = relevantSkills(this.skills, prompt).filter((match) => this.skillAutoDiscovery || match.explicit);
    const loadedSkills = [];
    let loadedBytes = 0;
    for (const skill of this.skills) {
      if (skill.selected !== true && !matches.some((match) => match.explicit && match.skill.id === skill.id)) continue;
      try {
        const full = skill.content ? skill : this.loadSkill?.(skill.id);
        if (!full?.content) continue;
        if (loadedBytes + Buffer.byteLength(full.content) > 60_000) {
          this.onInfo(`Skill ${skill.id} exceeds the per-turn guide budget; shorten or split its SKILL.md.`);
          continue;
        }
        loadedSkills.push(full);
        loadedBytes += Buffer.byteLength(full.content);
      } catch (error) { this.onInfo(`Could not load skill ${skill.id}: ${error.message}`); }
    }
    const hasSkillReader = availableTools.some((tool) => tool.name === "read_skill_resource");
    ...
    for (let turn = 0; turn < this.maxTurns; turn += 1) {
      ...
      const body = {
        ...
        instructions: [
          ...
          skillInstructions(loadedSkills),
          hasSkillReader && this.skillAutoDiscovery ? skillCatalogInstructions(matches, this.skills.length, availableTools.some((tool) => tool.name === "search_skills")) : "",
