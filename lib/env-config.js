...
const TOOL_ENV_KEYS = {
  ...
  read_skill_resource: "AI_HARNESS_TOOL_READ_SKILL_RESOURCE",
  search_skills: "AI_HARNESS_TOOL_SEARCH_SKILLS",
  ...
  delegate_to_sub_agent: "AI_HARNESS_TOOL_DELEGATE_TO_SUB_AGENT",
  edit_files: "AI_HARNESS_TOOL_EDIT_FILES",
  change_history: "AI_HARNESS_TOOL_CHANGE_HISTORY",
  javascript: "AI_HARNESS_TOOL_JAVASCRIPT",
};
...
export function applyEnvironmentSettings(uiStateStore, env = process.env, root = process.cwd()) {
  ...
  if (environmentDisablesFileAccess(env)) {
    next.toolPermissions.edit_files = false;
    next.toolPermissions.change_history = false;
    next.toolPermissions.javascript = false;
  }
