...
export function createExecutionControl({ onPaused = () => {}, onResumed = () => {} } = {}) {
  ...
  return {
    ...
    stop() {
      ...
      state = "stopped";
      abortController.abort(new DOMException("Run stopped", "AbortError"));
...
export function createWebSocketHandler({
  ...
  return function handleWebSocket(socket, req) {
      ...
      try {
        ...
        for (const message of decoded.messages) {
          ...
          if (payload.type === 'permission_response') {
            const pending = approvals.get(payload.id);
            if (pending && pending.sessionId === sessionId) pending.resolve(payload.approved === true);
            continue;
          }
          if (payload.type === "stop") {
            if (running && currentRun?.sessionId === sessionId) currentRun.controller.stop();
            continue;
          }
          ...
          if (payload.type === "reset") {
            ...
            agent?.reset();
            if (!agent) await resetContext?.(sessionId);
          ...
          void (async () => {
            try {
              const root = await run.controller.waitFor(resolveWorkspace(payload.workspace));
              ...
              if (!hasExistingAgent) {
                ...
                entry.promise = createAgentSession({
                  disabledSteps,
                  sessionId,
                  history: payload.history,
                  ask: (request, options = {}) => new Promise((resolve) => {
                    const signal = options.signal || sessionEntry.controller.signal;
                    ...
                    const finish = (approved) => {
                      ...
                      approvals.delete(id);
                      emit({ type: 'permission_closed', id, sessionId, approved, tool: request.tool, target: request.target });
                      resolve(approved);
                    };
                    const cancel = () => finish(false);
                    const timeout = setTimeout(cancel, 120000);
                    approvals.set(id, { sessionId, resolve: finish });
                    signal.addEventListener('abort', cancel, { once: true });
                    emit({ type: 'permission_request', id, sessionId, ...request });
                  }),
                  emit: (event) => {
                    if (!sessionEntry.controller.signal.aborted && currentRun?.controller === sessionEntry.controller) emit(event);
                  },
                  onTextDelta(textDelta) {
                    if (sessionEntry.controller.signal.aborted || currentRun?.controller !== sessionEntry.controller) return;
              ...
              const agent = await run.controller.waitFor(entry.promise);
              const composedPrompt = disabledSteps.includes("composer")
                ? payload.prompt
                : await run.controller.waitFor(agent.refinePrompt(payload.prompt, {
                  disabledSteps,
                  executionControl: run.controller,
                }));
              const prompt = hasExistingAgent || agent.context
                ? composedPrompt
                : promptWithHistory(composedPrompt, payload.history);
              streamStates.set(sessionId, { started: false });
              const textOutput = await run.controller.waitFor(agent.run({
                text: prompt,
                images: requestImages(payload),
              }, { disabledSteps, executionControl: run.controller }));
    ...
    socket.on("close", () => currentRun?.controller.stop());
