...
Skills live under `skills/<name>/SKILL.md` beside `db/`. Supporting files can live in `scripts/`, `references/`, `templates/`, `examples/`, `assets/`, and `agents/` inside each skill folder. The Skills modal lists names and descriptions first, then loads a full guide only when opened for editing. It can create or import a folder, edit supporting text files, and test metadata, references, and JavaScript syntax without running scripts. Selected skills remain a per-preset setting in SQLite and are loaded automatically for agent turns. For other skills, the agent sees a short list of likely matches based on the task, can use `search_skills` to find more, and calls `read_skill_resource` with `SKILL.md` to load the instructions when relevant. Supporting files are read separately as needed. Existing SQLite skills are exported to folders on startup; their old table is kept as `skills_legacy_archive` for recovery.

The bundled `/skills` tree includes guides from the neighboring Codex checkout. Supporting scripts and references needed by `imagegen`, `openai-docs`, `skill-creator`, and `skill-installer` are stored beside those guides. These files are optional and are never run automatically; their original license files are included in each skill folder.
